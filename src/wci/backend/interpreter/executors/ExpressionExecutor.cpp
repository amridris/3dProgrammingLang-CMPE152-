...
default:
return execute_binary_operator(node, node_type);
}
}
...
bool complex_integer_mode = false;
...
//Checking to see if the two operands are complex
else if ((typespec1 == Predefined::complex_type) && (typespec2 == Predefined::complex_type)) {
//set oomplex mode true
complex_mode = true;
}

else if((typespec1 == Predefined::complex_type) && (typespec2 == Predefined::real_type))
{
complex_integer_mode = true;
}
...
default:
    return Object();  // empty -- shouldn't get here
}
} else if (complex_mode) {
    Object attribute1 = operand_node1->get_attribute((ICodeKey) ID);
    Object attribute2 = operand_node2->get_attribute((ICodeKey) ID);
    SymTabEntry *variable_id1 = cast(attribute1, SymTabEntry*);
    SymTabEntry *variable_id2 = cast(attribute2, SymTabEntry*);
    string variable_name1 = variable_id1->get_name();
    string variable_name2 = variable_id2->get_name();
    TypeSpec *variable_typespec1 = variable_id1->get_typespec();
    TypeSpec *variable_typespec2 = variable_id2->get_typespec();

    int nesting_level1 = variable_id1->get_symtab()->get_nesting_level();
    int nesting_level2 = variable_id2->get_symtab()->get_nesting_level();

//accessing field within record type reference
    ActivationRecord *ar1 = runtime_stack->get_topmost(nesting_level1);
    ActivationRecord *ar2 = runtime_stack->get_topmost(nesting_level2);
    Cell *variable_cell1 = ar1->get_cell(variable_name1);
    Cell *variable_cell2 = ar2->get_cell(variable_name2);
//
Object cell_value1 = variable_cell1->get_value();
MemoryMapImpl *mmap1 = cast(cell_value1, MemoryMapImpl*);
Cell *re_cell1 = mmap1->get_cell("re");
Cell *im_cell1 = mmap1->get_cell("im");
Object re_value1 = re_cell1->get_value();
Object im_value1 = im_cell1->get_value();

Object cell_value2 = variable_cell2->get_value();
MemoryMapImpl *mmap2 = cast(cell_value2, MemoryMapImpl*);
Cell *re_cell2 = mmap2->get_cell("re");
Cell *im_cell2 = mmap2->get_cell("im");
Object re_value2 = re_cell2->get_value();
Object im_value2 = im_cell2->get_value();
//each modifier represents real(0) or imaginary(1) part of complex variable

    float realValue1 = cast(re_value1, float);
    float realValue2 = cast(re_value2, float);
    float imagValue1 = cast(im_value1, float);
    float imagValue2 = cast(im_value2, float);

    float realResult, imagResult;
    ...
    switch(node_type){
        case NT_ADD:
        {
            realResult = realValue1 + realValue2;
            imagResult = imagValue1 + imagValue2;
            result.push_back(realResult);
            result.push_back(imagResult);
        ...
        case NT_MULTIPLY:
        {
            ...
            realResult = (realValue1*realValue2) - (imagValue1*imagValue2);
            imagResult = (realValue1*imagValue2) + (imagValue1*realValue2);
            result.push_back(realResult);
            result.push_back(imagResult);
        ...
        case NT_FLOAT_DIVIDE:
        {
            int denom = (realValue2*realValue2) + (imagValue2*imagValue2);
            if(denom == 0)
            {
                error_handler.flag(node, DIVISION_BY_ZERO, this);
            }

            realResult = ((realValue1*realValue2) + (imagValue1*imagValue2)) / denom;
            imagResult = (imagValue1*realValue2) - (realValue1*imagValue2) / denom;
            result.push_back(realResult);
            result.push_back(imagResult);
