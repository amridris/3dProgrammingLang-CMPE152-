                                      ...
                                      TypeSpec *value_typespec)
{
    ...
    // Convert an integer value to real if necessary.
    else if (   (target_typespec == Predefined::complex_type)
        && (value_typespec  == Predefined::complex_type))
    {
        vector<float> vec = cast(cell_value, vector<float>);
        float re_value = vec[0];
        float im_value = vec[1];

        Object cell_value1 = target_cell->get_value();
        MemoryMapImpl *mmap1 = cast(cell_value1, MemoryMapImpl*);
        Cell *re_cell = mmap1->get_cell("re");
        Cell *im_cell = mmap1->get_cell("im");
        re_cell->set_value(re_value);
        im_cell->set_value(im_value);
